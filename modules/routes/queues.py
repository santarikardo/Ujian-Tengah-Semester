...
async def get_all_queues(
    ...
    if current_user.role == UserRole.PATIENT:
        queues = queue_crud.read_all_queues(
            patient_id=current_user.id, status=status
        )
    else:
        queues = queue_crud.read_all_queues(
...
async def call_queue(
    queue_id: str, current_user: User = Depends(require_doctor_or_admin)
):
    queue = queue_crud.read_queue(queue_id)
    if not queue:
        raise HTTPException(status_code=404, detail="Antrean tidak ditemukan")
    ...
    updated_queue = queue_crud.update_queue_status(
        queue_id, QueueStatus.IN_SERVICE
    )
    return {"message": "Pasien berhasil dipanggil", "queue": updated_queue}


# COMPLETE QUEUE – pakai parameter ala Kaggle di request body
...
async def cancel_queue(queue_id: str, current_user: User = Depends(get_current_user)):
    ...
    updated_queue = queue_crud.update_queue_status(
        queue_id, QueueStatus.CANCELLED
    )
